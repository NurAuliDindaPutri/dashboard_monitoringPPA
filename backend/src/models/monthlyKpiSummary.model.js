...
async function findAll(filter = {}) {
    const { site_id, period_year, period_month } = filter;
    let sql = `
    SELECT mks.*, s.site_code, s.site_name
    FROM monthly_kpi_summary mks
    JOIN sites s ON s.id = mks.site_id
    WHERE 1 = 1
  `;
    const params = [];
    ...
    if (period_year) {
        sql += ' AND mks.period_year = ?';
        params.push(period_year);
    }
    if (period_month) {
        sql += ' AND mks.period_month = ?';
        params.push(period_month);
    }

    sql += ' ORDER BY mks.period_year DESC, mks.period_month DESC, s.site_code ASC';
...
async function update(id, data) {
    ...
    await pool.query(
     ...
     SET site_id = ?, period_year = ?, period_month = ?,
...
async function remove(id) {
    const [result] = await pool.query(
        'DELETE FROM monthly_kpi_summary WHERE id = ?',
        [id]
    );
